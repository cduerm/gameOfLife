...
// Game holds all data of a Game of Life.
type Game struct {
	field                [][]bool
	row, col             int
	step                 int
...
func New(row, col int, ruleCreate, ruleLive GolRule, borderStyle BoundaryCondition, p float32) *Game {
	g := new(Game)
	g.row = row
	g.col = col
...
// SetCell changes the value of the specified cell if it is inside the domain.
func (g *Game) SetCell(i, j int, val bool) {
	if i >= 0 && i < g.row && j >= 0 && j < g.col {
		g.field[i][j] = val
	}
}

// ToggleCell switches the value of the specified cell if it is inside the domain.
func (g *Game) ToggleCell(i, j int) {
	if i >= 0 && i < g.row && j >= 0 && j < g.col {
		g.field[i][j] = !g.field[i][j]
	}
}

// Field returns the slice containing all cells, but not the boundary
func (g *Game) Field() [][]bool {
	return g.field
}

// GetRows returns the number of rows.
func (g *Game) Rows() int { return g.row }

// GetCols returns the number of colons.
func (g *Game) Cols() int { return g.col }

func (g *Game) neighbours(i, j int) int {
	n := 0
	if g.Cell(i, j) {
		n--
	}
